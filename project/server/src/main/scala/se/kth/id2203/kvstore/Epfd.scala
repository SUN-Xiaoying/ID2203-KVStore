...
import se.kth.id2203.leader.CheckTimeout
...
class EPFD extends ComponentDefinition {
      ...
      for (p <- currentTopology) {
        if (!alive.contains(p) && !suspected.contains(p)) {
          ...
          if(debugSource.isDefined)
            trigger(NetMessage(self, debugSource.get, Suspect(p)) -> pl)
        } else if (alive.contains(p) && suspected.contains(p)) {
          suspected = suspected - p;
            println(s"$self EFPD restored $p")
            trigger(Restore(p) -> epfd);
        }
          trigger(NetMessage(self, p, HeartbeatRequest(seqnum)) -> pl);
      }
  ...
  pl uponEvent {
    case NetMessage(src, HeartbeatRequest(seq)) => {
      trigger(NetMessage(self, src.src, HeartbeatReply(seq)) -> pl);
    }
